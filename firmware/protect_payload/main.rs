...
fn try_rw_miralis() {
    ...
    assert_eq!(
        mcause, 0x7,
        "exception was not store access fault while storing to Miralis"
    );
}
...
fn try_rw_protected_payload() {
    ...
    assert_eq!(
        mcause, 0x7,
        "exception was not store access fault while storing to payload"
    );
}
