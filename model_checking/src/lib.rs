...
pub fn requires_interrupt_injection() {
    ...
    let has_miralis_interrupt = if let Some(_) = ctx.has_pending_interrupt() {
        true
    } else {
        false
    };

    let has_sail_interrupt = if let Some(_) = dispatchInterrupt(&mut sail_ctx, Privilege::Machine) {
        true
    } else {
        false
    };
    ...
    assert_eq!(
        has_miralis_interrupt, has_sail_interrupt,
