...
use sail_model::{Privilege, SailVirtContext};
use sail_prelude::BitVector;

pub fn miralis_to_sail(ctx: &VirtContext) -> SailVirtContext {
    SailVirtContext {
        mepc: BitVector::new(ctx.csr.mepc as u64),
        sepc: BitVector::new(ctx.csr.sepc as u64),
        misa: BitVector::new(ctx.csr.misa as u64),
...
pub fn sail_to_miralis(sctx: SailVirtContext) -> VirtContext {
    ...
    ctx.csr.mepc = sctx.mepc.bits() as usize;
    ctx.csr.sepc = sctx.sepc.bits() as usize;
    ctx.csr.misa = sctx.misa.bits() as usize;
    ctx.csr.mstatus = sctx.mstatus.bits() as usize;

    ctx.mode = match sctx.cur_privilege {
        Privilege::User => Mode::U,
        Privilege::Supervisor => Mode::S,
        Privilege::Machine => Mode::M,
    };

    ctx.pc = sctx.next_pc.bits() as usize;
...
mod tests {
    ...
    fn simple_mret() {
        ...
        sail_model::execute_MRET(&mut ctx);
...
#[cfg(kani)]
mod verification {
    use miralis::arch::{mstatus, Arch, Architecture};
