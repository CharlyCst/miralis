...
pub struct SailVirtContext {
    pub mepc: BitVector<64>,
    pub sepc: BitVector<64>,
    pub misa: BitVector<64>,
    pub mstatus: BitVector<64>,
    pub next_pc: BitVector<64>,
    pub pc: BitVector<64>,
...
fn haveUsrMode(ctx: &mut SailVirtContext) -> bool {
    ctx.misa.bits() & (1 << 20) != 0
}
...
fn exception_handler(
    ...
    ctx.mstatus = ctx
        .mstatus
        .set_subrange::<11, 13, 2>(privLevel_to_bits(if haveUsrMode(ctx) {
            Privilege::User
        } else {
            Privilege::Machine
        }));
