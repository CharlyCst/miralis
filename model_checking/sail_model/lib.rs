...
pub fn findPendingInterrupt(
    ...
    } else if { (_get_Minterrupts_UEI(sail_ctx, ip) == BitVector::<1>::new(0b1)) } {
        Some(InterruptType::I_U_External)
    } else if { (_get_Minterrupts_USI(sail_ctx, ip) == BitVector::<1>::new(0b1)) } {
        Some(InterruptType::I_U_Software)
    } else if { (_get_Minterrupts_UTI(sail_ctx, ip) == BitVector::<1>::new(0b1)) } {
        Some(InterruptType::I_U_Timer)
    } else {
        None
    }
}
