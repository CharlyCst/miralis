...
use crate::ace::confidential_flow::handlers::symmetrical_multiprocessing::{
    Ipi, NoOperation, RemoteFenceI, RemoteSfenceVma, RemoteSfenceVmaAsid, SbiHsmHartStart,
    SbiHsmHartStatus, SbiHsmHartStop, SbiHsmHartSuspend,
};
use crate::ace::confidential_flow::handlers::virtual_instructions::VirtualInstruction;
use crate::ace::confidential_flow::{ApplyToConfidentialHart, DeclassifyToConfidentialVm};
...
use crate::ace::core::architecture::{HartLifecycleState, TrapCause};
use crate::ace::core::control_data::{
    ConfidentialHart, ConfidentialHartRemoteCommand, ConfidentialVmId, ControlDataStorage,
    HardwareHart, HypervisorHart, ResumableOperation,
};
use crate::ace::error::Error;
use crate::ace::non_confidential_flow::{DeclassifyToHypervisor, NonConfidentialFlow};
use crate::debug;
...
impl<'a> ConfidentialFlow<'a> {
    const CTX_SWITCH_ERROR_MSG: &'static str =
        "Bug: Invalid argument provided by the assembly context switch";
    const DUMMY_HART_ERROR_MSG: &'static str =
        "Bug: Found dummy hart instead of a confidential hart";
    ...
    unsafe extern "C" fn route_trap_from_confidential_hart(
        ...
        let flow = Self {
            hardware_hart: unsafe {
                hardware_hart_pointer
                    .as_mut()
                    .expect(Self::CTX_SWITCH_ERROR_MSG)
            },
        };
        ...
        match TrapCause::from_hart_architectural_state(
            ...
            Interrupt => {
                HandleInterrupt::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetSpecVersion)) => {
                SbiGetSpecVersion::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetImplId)) => {
                SbiGetImplId::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetImplVersion)) => {
                SbiGetImplVersion::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(ProbeExtension)) => {
                SbiExtensionProbe::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetMvendorId)) => {
                SbiGetMvendorid::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetMarchid)) => {
                SbiGetMarchId::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Base(GetMimpid)) => {
                SbiGetMimpid::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Ipi(SendIpi)) => {
                Ipi::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteFenceI)) => {
                RemoteFenceI::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteSfenceVma)) => {
                RemoteSfenceVma::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteSfenceVmaAsid)) => {
                RemoteSfenceVmaAsid::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteHfenceGvmaVmid)) => {
                NoOperation::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteHfenceGvma)) => {
                NoOperation::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteHfenceVvmaAsid)) => {
                NoOperation::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Rfence(RemoteHfenceVvma)) => {
                NoOperation::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Hsm(HartStart)) => {
                SbiHsmHartStart::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Hsm(HartStop)) => {
                SbiHsmHartStop::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Hsm(HartSuspend)) => {
                SbiHsmHartSuspend::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Hsm(HartGetStatus)) => {
                SbiHsmHartStatus::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Srst(SystemReset)) => {
                ShutdownRequest::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Covg(AddMmioRegion)) => {
                AddMmioRegion::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Covg(RemoveMmioRegion)) => {
                RemoveMmioRegion::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VsEcall(Covg(AllowExternalInterrupt)) => {
                AllowExternalInterrupt::from_confidential_hart(flow.confidential_hart())
                    .handle(flow)
            }
            VsEcall(Covg(ShareMemory)) => {
                SharePageRequest::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            ...
            GuestLoadPageFault => {
                MmioLoadRequest::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            VirtualInstruction => {
                VirtualInstruction::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            GuestStorePageFault => {
                MmioStoreRequest::from_confidential_hart(flow.confidential_hart()).handle(flow)
            }
            _trap_reason => {
    ...
    pub fn into_non_confidential_flow(self) -> NonConfidentialFlow<'a> {
        ...
        ControlDataStorage::try_confidential_vm(self.confidential_vm_id(), |mut confidential_vm| {
            ...
            Ok(NonConfidentialFlow::create(self.hardware_hart)
                .declassify_to_hypervisor_hart(declassifier))
        })
    ...
    pub fn resume_confidential_hart_execution(mut self) -> ! {
        ...
        if self.confidential_hart().lifecycle_state() == &HartLifecycleState::PoweredOff {
            crate::ace::confidential_flow::handlers::shutdown::shutdown_confidential_hart(self);
        }
        ...
        use crate::ace::core::control_data::ResumableOperation::*;
        match self.confidential_hart_mut().take_resumable_operation() {
            Some(SbiRequest()) => {
                SbiResponse::from_hypervisor_hart(self.hypervisor_hart()).handle(self)
            }
            Some(ResumeHart(v)) => v.handle(self),
            Some(MmioLoad(v)) => {
                MmioLoadResponse::from_hypervisor_hart(self.hypervisor_hart(), v).handle(self)
            }
            Some(MmioStore(v)) => {
                MmioStoreResponse::from_hypervisor_hart(self.hypervisor_hart(), v).handle(self)
            }
            Some(SharePage(v)) => {
                SharePageComplete::from_hypervisor_hart(self.hypervisor_hart(), v).handle(self)
            }
            None => self.exit_to_confidential_hart(),
        }
    }

    pub fn declassify_to_confidential_hart(
        mut self,
        declassifier: DeclassifyToConfidentialVm,
    ) -> Self {
        match declassifier {
            DeclassifyToConfidentialVm::SbiResponse(v) => {
                v.declassify_to_confidential_hart(self.confidential_hart_mut())
            }
            DeclassifyToConfidentialVm::MmioLoadResponse(v) => {
                v.declassify_to_confidential_hart(self.confidential_hart_mut())
            }
            DeclassifyToConfidentialVm::MmioStoreResponse(v) => {
                v.declassify_to_confidential_hart(self.confidential_hart_mut())
            }
            DeclassifyToConfidentialVm::Resume(v) => {
                v.declassify_to_confidential_hart(self.confidential_hart_mut())
            }
        }
        self
    ...
    pub fn declassify_and_exit_to_confidential_hart(
        self,
        declassifier: DeclassifyToConfidentialVm,
    ) -> ! {
        self.declassify_to_confidential_hart(declassifier)
            .exit_to_confidential_hart()
    }
    ...
    pub fn apply_and_exit_to_confidential_hart(
        mut self,
        transformation: ApplyToConfidentialHart,
    ) -> ! {
        match transformation {
            ApplyToConfidentialHart::MmioAccessFault(v) => {
                v.apply_to_confidential_hart(self.confidential_hart_mut())
            }
            ApplyToConfidentialHart::SbiResponse(v) => {
                v.apply_to_confidential_hart(self.confidential_hart_mut())
            }
            ApplyToConfidentialHart::VirtualInstruction(v) => {
                v.apply_to_confidential_hart(self.confidential_hart_mut())
            }
        }
...
impl<'a> ConfidentialFlow<'a> {
    ...
    pub fn broadcast_remote_command(
        &mut self,
        confidential_hart_remote_command: ConfidentialHartRemoteCommand,
    ) -> Result<(), Error> {
                    ...
                    |ref mut confidential_hart_remote_commands| {
                        confidential_hart_remote_commands.drain(..).for_each(
                            |confidential_hart_remote_command| {
                                ...
                                self.confidential_hart_mut()
                                    .execute(&confidential_hart_remote_command);
                            },
                        );
                        Ok(())
                    },
...
impl<'a> ConfidentialFlow<'a> {
    ...
    pub fn suspend_confidential_hart(&mut self) -> Result<(), Error> {
        self.confidential_hart_mut()
            .transition_from_started_to_suspended()
    }
    ...
    pub fn stop_confidential_hart(&mut self) -> Result<(), Error> {
        self.confidential_hart_mut()
            .transition_from_started_to_stopped()
    }

    /// Delegates the state transition to the confidential hart. The confidential hart is intentionally encapsulated to prevent access to it
    /// other than via the ControlFlow.
    pub fn resume_confidential_hart(
        &mut self,
        start_address: usize,
        opaque: usize,
    ) -> Result<(), Error> {
        self.confidential_hart_mut()
            .transition_from_suspended_to_started(start_address, opaque)
    }
...
impl<'a> ConfidentialFlow<'a> {
    pub fn set_resumable_operation(mut self, request: ResumableOperation) -> Self {
        self.confidential_hart_mut()
            .set_resumable_operation(request);
        self
    }
    ...
    pub fn confidential_vm_id(&'a self) -> ConfidentialVmId {
        self.confidential_hart()
            .confidential_vm_id()
            .expect(Self::DUMMY_HART_ERROR_MSG)
    }
