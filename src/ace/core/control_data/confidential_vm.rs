...
impl ConfidentialVm {
    ...
    pub fn steal_confidential_hart(
        ...
        // The hypervisor might try to schedule a confidential hart that has never been started. This is forbidden.
            ...
            confidential_hart.is_executable(),
            Error::HartNotExecutable()
        )?;
...
impl ConfidentialVm {
    pub fn are_all_harts_shutdown(&self) -> bool {
        self.confidential_harts
            .iter()
            .filter(|hart| hart.lifecycle_state() != &HartLifecycleState::PoweredOff)
            .count()
            == 0
    }

    /// Returns the lifecycle state of the confidential hart
    pub fn confidential_hart_lifecycle_state(
        &self,
        confidential_hart_id: usize,
    ) -> Result<HartLifecycleState, Error> {
        ensure!(
            confidential_hart_id < self.confidential_harts.len(),
            Error::InvalidHartId()
        )?;
        Ok(self.confidential_harts[confidential_hart_id]
            .lifecycle_state()
            .clone())
    }
