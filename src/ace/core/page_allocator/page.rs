...
use crate::ace::core::memory_layout::{
    ConfidentialMemoryAddress, MemoryLayout, NonConfidentialMemoryAddress,
};
...
impl Page<UnAllocated> {
    ...
    pub fn copy_from_non_confidential_memory(
        mut self,
        mut address: NonConfidentialMemoryAddress,
    ...
    pub unsafe fn merge(mut from_pages: Vec<Page<UnAllocated>>, new_size: PageSize) -> Self {
        ...
        assert!(
            from_pages[0].start_address() + new_size.in_bytes()
                == from_pages[from_pages.len() - 1].end_address()
        );
...
impl Page<Allocated> {
    ...
    pub fn read(&self, offset_in_bytes: usize) -> Result<usize, Error> {
        ...
        let data = unsafe {
            ...
            let pointer = self
                .address
                .add(offset_in_bytes, self.end_address_ptr())
                .unwrap();
...
impl<T: PageState> Page<T> {
    ...
    pub fn write(&mut self, offset_in_bytes: usize, value: usize) -> Result<(), Error> {
            ...
            offset_in_bytes % Self::ENTRY_SIZE == 0,
    ...
    pub fn measure(&self, digest: &mut MeasurementDigest, guest_physical_address: usize) {
        ...
        let slice: &[u8] =
            unsafe { core::slice::from_raw_parts(self.address().to_ptr(), self.size().in_bytes()) };
        hasher.update(&slice);
