...
impl PromoteToConfidentialVm {
    ...
    pub fn from_hypervisor_hart(hypervisor_hart: &HypervisorHart) -> Self {
        let fdt_address = ConfidentialVmPhysicalAddress::new(
    ...
    fn process_device_tree(
        ...
        let address_in_confidential_memory =
            memory_protector.translate_address(&self.fdt_address)?;
        ...
        // the page boundary. These 8 bytes should contain the `magic` (first 4 bytes) and `size` (next 4 bytes).
            ...
            address_in_confidential_memory.is_aligned_to(Self::FDT_ALIGNMENT_IN_BYTES),
            Error::AuthBlobNotAlignedTo64Bits()
        )?;
        ...
        let fdt_total_size =
            unsafe { FlattenedDeviceTree::total_size(address_in_confidential_memory.to_ptr())? };
        ensure!(
            fdt_total_size >= FlattenedDeviceTree::FDT_HEADER_SIZE,
            Error::FdtInvalidSize()
        )?;

        // To work with FDT, we must have it as a continous chunk of memory. We accept only FDTs that fit within 2MiB
        ensure!(
            fdt_total_size.div_ceil(PageSize::Size2MiB.in_bytes()) == 1,
            Error::FdtInvalidSize()
        )?;
        ...
        let number_of_confidential_harts =
            match unsafe { FlattenedDeviceTree::from_raw_pointer(large_page.address().to_ptr()) } {
        ...
        PageAllocator::release_pages(alloc::vec![large_page.deallocate()]);
            ...
            number_of_confidential_harts > 0,
    ...
    fn authenticate_and_authorize_vm(
        ...
        if let Some(blob_address) = self.auth_blob_address {
            ...
            let address_in_confidential_memory =
                memory_protector.translate_address(&blob_address)?;
            // Make sure that the address is 8-bytes aligned. Once we ensure this, we can safely read 8 bytes because they must be within
            // the page boundary. These 8 bytes should contain the `magic` (first 4 bytes) and `size` (next 4 bytes).
            ensure!(
                address_in_confidential_memory.is_aligned_to(Self::TAP_ALIGNMENT_IN_BYTES),
                Error::AuthBlobNotAlignedTo64Bits()
            )?;
            // Below use of unsafe is ok because (1) the security monitor owns the memory region containing the data of the not-yet-created
            // confidential VM's and (2) there is only one physical hart executing this code.
            ...
            // that fit within 2MiB
                ...
                auth_blob_total_size.div_ceil(PageSize::Size2MiB.in_bytes()) == 1,
                Error::AuthBlobInvalidSize()
            )?;
    ...
    fn relocate(
        ...
        number_of_bytes_to_copy: usize,
            ...
            (base_address.usize() as *const u8).is_aligned_to(core::mem::size_of::<usize>()),
        ...
        while copied_bytes < number_of_bytes_to_copy {
            ...
            let confidential_memory_address =
                memory_protector.translate_address(&confidential_vm_physical_address)?;
