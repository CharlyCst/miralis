...
use crate::arch::{
    get_raw_faulting_instr, mie, mstatus, Arch, Architecture, Csr, MCause, Mode, Register,
    PAGE_SIZE,
};
...
impl OffloadPolicy {
    ...
    fn redirect_trap_to_s_mode(ctx: &mut VirtContext) -> PolicyHookResult {
        ...
        let prev_virt: bool = mstatus & MPV_FILTER != 0;
        ...
        let mut next_virt: bool = false;

        // Sanity check on previous mode
        let prev_mode = (mstatus & MPP_FILTER) >> MPP_OFFSET;
        if prev_mode != Mode::S as usize && prev_mode != Mode::U as usize {
            unreachable!()
        }
        ...
        if ctx.extensions.has_h_extension && !next_virt {
            ...
            if prev_virt {
                ...
                hstatus &= !SPV_FILTER;
                hstatus |= if prev_virt { SPV_FILTER } else { 0 };
                hstatus &= !GVA_FILTER;
