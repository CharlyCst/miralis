...
pub fn _update_Sinterrupts_LCOFIE(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
...
pub fn lower_mip(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts) -> Sinterrupts {
    let s: Sinterrupts = {
        let var_173 = zero_extend_64(BitVector::<1>::new(0b0));
        Mk_Sinterrupts(sail_ctx, var_173)
    };
    let s = {
        let var_171 = s;
        let var_172 = (_get_Minterrupts_SEI(sail_ctx, m) & _get_Minterrupts_SEI(sail_ctx, d));
        _update_Sinterrupts_SEI(sail_ctx, var_171, var_172)
    };
    let s = {
        let var_169 = s;
        let var_170 = (_get_Minterrupts_STI(sail_ctx, m) & _get_Minterrupts_STI(sail_ctx, d));
        _update_Sinterrupts_STI(sail_ctx, var_169, var_170)
    };
    let s = {
        let var_167 = s;
        let var_168 = (_get_Minterrupts_SSI(sail_ctx, m) & _get_Minterrupts_SSI(sail_ctx, d));
        _update_Sinterrupts_SSI(sail_ctx, var_167, var_168)
    };
    let s = {
        let var_165 = s;
        let var_166 = (_get_Minterrupts_LCOFIE(sail_ctx, m) & _get_Minterrupts_LCOFIE(sail_ctx, d));
        _update_Sinterrupts_LCOFIE(sail_ctx, var_165, var_166)
    };
    s
}
...
pub fn lower_mie(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts) -> Sinterrupts {
    let s: Sinterrupts = {
        let var_180 = zero_extend_64(BitVector::<1>::new(0b0));
        Mk_Sinterrupts(sail_ctx, var_180)
    };
    let s = {
        let var_178 = s;
        let var_179 = (_get_Minterrupts_SEI(sail_ctx, m) & _get_Minterrupts_SEI(sail_ctx, d));
        _update_Sinterrupts_SEI(sail_ctx, var_178, var_179)
    };
    let s = {
        let var_176 = s;
        let var_177 = (_get_Minterrupts_STI(sail_ctx, m) & _get_Minterrupts_STI(sail_ctx, d));
        _update_Sinterrupts_STI(sail_ctx, var_176, var_177)
    };
    let s = {
        let var_174 = s;
        let var_175 = (_get_Minterrupts_SSI(sail_ctx, m) & _get_Minterrupts_SSI(sail_ctx, d));
        _update_Sinterrupts_SSI(sail_ctx, var_174, var_175)
    };
    s
}
...
pub fn lift_sip(sail_ctx: &mut SailVirtCtx, o: Minterrupts, d: Minterrupts, s: Sinterrupts) -> Minterrupts {
    ...
    let m = if {(_get_Minterrupts_SSI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_185 = m;
            let var_186 = _get_Sinterrupts_SSI(sail_ctx, s);
            _update_Minterrupts_SSI(sail_ctx, var_185, var_186)
        }
    } else {
        m
    };
    if {haveNExt(sail_ctx, ())} {
        let m = if {(_get_Minterrupts_UEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_183 = m;
                let var_184 = _get_Sinterrupts_UEI(sail_ctx, s);
                _update_Minterrupts_UEI(sail_ctx, var_183, var_184)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_USI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_181 = m;
                let var_182 = _get_Sinterrupts_USI(sail_ctx, s);
                _update_Minterrupts_USI(sail_ctx, var_181, var_182)
            }
        } else {
            m
        };
...
pub fn legalize_sip(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts, v: BitVector<64>) -> Minterrupts {
    {
        let var_187 = m;
        let var_188 = d;
        let var_189 = Mk_Sinterrupts(sail_ctx, v);
        lift_sip(sail_ctx, var_187, var_188, var_189)
    }
}
...
pub fn lift_sie(sail_ctx: &mut SailVirtCtx, o: Minterrupts, d: Minterrupts, s: Sinterrupts) -> Minterrupts {
    ...
    let m = if {(_get_Minterrupts_SEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_200 = m;
            let var_201 = _get_Sinterrupts_SEI(sail_ctx, s);
            _update_Minterrupts_SEI(sail_ctx, var_200, var_201)
        }
    } else {
        m
    };
    let m = if {(_get_Minterrupts_STI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_198 = m;
            let var_199 = _get_Sinterrupts_STI(sail_ctx, s);
            _update_Minterrupts_STI(sail_ctx, var_198, var_199)
        }
    } else {
        m
    };
    let m = if {(_get_Minterrupts_SSI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_196 = m;
            let var_197 = _get_Sinterrupts_SSI(sail_ctx, s);
            _update_Minterrupts_SSI(sail_ctx, var_196, var_197)
        }
    } else {
        m
    };
    if {haveNExt(sail_ctx, ())} {
        let m = if {(_get_Minterrupts_UEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_194 = m;
                let var_195 = _get_Sinterrupts_UEI(sail_ctx, s);
                _update_Minterrupts_UEI(sail_ctx, var_194, var_195)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_UTI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_192 = m;
                let var_193 = _get_Sinterrupts_UTI(sail_ctx, s);
                _update_Minterrupts_UTI(sail_ctx, var_192, var_193)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_USI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_190 = m;
                let var_191 = _get_Sinterrupts_USI(sail_ctx, s);
                _update_Minterrupts_USI(sail_ctx, var_190, var_191)
            }
        } else {
            m
        };
...
pub fn legalize_sie(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts, v: BitVector<64>) -> Minterrupts {
    {
        let var_202 = m;
        let var_203 = d;
        let var_204 = Mk_Sinterrupts(sail_ctx, v);
        lift_sie(sail_ctx, var_202, var_203, var_204)
    }
}
...
pub fn legalize_satp64(sail_ctx: &mut SailVirtCtx, a: Architecture, o: BitVector<64>, v: BitVector<64>) -> BitVector<64> {
    ...
    match {
        let var_205 = a;
        let var_206 = _get_Satp64_Mode(sail_ctx, s);
        satp64Mode_of_bits(sail_ctx, var_205, var_206)
    } {
        None => {o}
...
pub fn legalize_menvcfg(sail_ctx: &mut SailVirtCtx, o: MEnvcfg, v: BitVector<64>) -> MEnvcfg {
    ...
    let o = {
        let var_207 = o;
        let var_208 = if {sys_enable_writable_fiom(())} {
            _get_MEnvcfg_FIOM(sail_ctx, v)
        } else {
            BitVector::<1>::new(0b0)
        };
        _update_MEnvcfg_FIOM(sail_ctx, var_207, var_208)
    };
    o
}
...
pub fn legalize_senvcfg(sail_ctx: &mut SailVirtCtx, o: SEnvcfg, v: BitVector<64>) -> SEnvcfg {
    ...
    let o = {
        let var_209 = o;
        let var_210 = if {sys_enable_writable_fiom(())} {
            _get_SEnvcfg_FIOM(sail_ctx, v)
        } else {
            BitVector::<1>::new(0b0)
        };
        _update_SEnvcfg_FIOM(sail_ctx, var_209, var_210)
    };
    o
}
    ...
    match bitvector_access(match_type, 1) {
        true if {(G >= 2)} => {{
            let mask: xlenbits = zero_extend_64({
                let var_211 = min_int((G - 1), 64);
                ones::<64>(sail_ctx, var_211)
            });
            (addr | mask)
        }}
        false if {(G >= 1)} => {{
            let mask: xlenbits = zero_extend_64({
                let var_212 = min_int(G, 64);
                ones::<64>(sail_ctx, var_212)
            });
            (addr & !(mask))
        }}
        _ => {addr}
...
pub fn pmpTORLocked(sail_ctx: &mut SailVirtCtx, cfg: Pmpcfg_ent) -> bool {
    ((_get_Pmpcfg_ent_L(sail_ctx, cfg) == BitVector::<1>::new(0b1)) && ({
        let var_213 = _get_Pmpcfg_ent_A(sail_ctx, cfg);
        pmpAddrMatchType_of_bits(sail_ctx, var_213)
...
pub fn pmpWriteCfg(sail_ctx: &mut SailVirtCtx, n: usize, cfg: Pmpcfg_ent, v: BitVector<8>) -> Pmpcfg_ent {
    if {pmpLocked(sail_ctx, cfg)} {
        ...
        let cfg = {
            let var_221 = (v & BitVector::<8>::new(0b10011111));
            Mk_Pmpcfg_ent(sail_ctx, var_221)
        };
        let cfg = if {((_get_Pmpcfg_ent_W(sail_ctx, cfg) == BitVector::<1>::new(0b1)) && (_get_Pmpcfg_ent_R(sail_ctx, cfg) == BitVector::<1>::new(0b0)))} {
            {
                let var_217 = {
                    let var_219 = _update_Pmpcfg_ent_X(sail_ctx, cfg, BitVector::<1>::new(0b0));
                    let var_220 = BitVector::<1>::new(0b0);
                    _update_Pmpcfg_ent_W(sail_ctx, var_219, var_220)
                };
                let var_218 = BitVector::<1>::new(0b0);
                _update_Pmpcfg_ent_R(sail_ctx, var_217, var_218)
            }
        } else {
            cfg
        };
        let cfg = if {((sys_pmp_grain(()) >= 1) && ({
            let var_216 = _get_Pmpcfg_ent_A(sail_ctx, cfg);
            pmpAddrMatchType_of_bits(sail_ctx, var_216)
        } == PmpAddrMatchType::NA4))} {
            {
                let var_214 = cfg;
                let var_215 = pmpAddrMatchType_to_bits(sail_ctx, PmpAddrMatchType::OFF);
                _update_Pmpcfg_ent_A(sail_ctx, var_214, var_215)
            }
        } else {
            cfg
        };
    ...
    if {(64 == 32)} {
        ...
        for i in 0..=7 {
            ...
            sail_ctx.pmpcfg_n[idx] = {
                let var_222 = idx;
                let var_223 = sail_ctx.pmpcfg_n[idx];
                let var_224 = subrange_bits_8(v, ((8 * i) + 7), (8 * i));
                pmpWriteCfg(sail_ctx, var_222, var_223, var_224)
            }
        }
    }
}
...
pub fn pmpWriteAddrReg(sail_ctx: &mut SailVirtCtx, n: usize, v: BitVector<64>) {
    sail_ctx.pmpaddr_n[n] = {
        let var_228 = pmpLocked(sail_ctx, sail_ctx.pmpcfg_n[n]);
        let var_229 = if {((n + 1) < 64)} {
            pmpTORLocked(sail_ctx, sail_ctx.pmpcfg_n[(n + 1)])
        } else {
            false
        };
        let var_230 = sail_ctx.pmpaddr_n[n];
        let var_231 = v;
        pmpWriteAddr(sail_ctx, var_228, var_229, var_230, var_231)
    }
}
...
pub fn check_CSR_access(sail_ctx: &mut SailVirtCtx, csrrw: BitVector<2>, csrpr: BitVector<2>, p: Privilege, isWrite: bool) -> bool {
    (!(((isWrite == true) && (csrrw == BitVector::<2>::new(0b11)))) && {
        let var_232 = privLevel_to_bits(sail_ctx, p);
        let var_233 = csrpr;
        _operator_biggerequal_u_(sail_ctx, var_232, var_233)
    })
}
...
pub fn check_CSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege, isWrite: bool) -> bool {
    (is_CSR_defined(sail_ctx, csr, p) && ({
        let var_234 = csrAccess(sail_ctx, csr);
        let var_235 = csrPriv(sail_ctx, csr);
        let var_236 = p;
        let var_237 = isWrite;
        check_CSR_access(sail_ctx, var_234, var_235, var_236, var_237)
...
pub fn exception_delegatee(sail_ctx: &mut SailVirtCtx, e: ExceptionType, p: Privilege) -> Privilege {
    ...
    let _super_ = {
        let var_241 = bitvector_access(sail_ctx.medeleg.bits, idx);
        bit_to_bool(sail_ctx, var_241)
    };
    let user = if {haveSupMode(sail_ctx, ())} {
        (_super_ && (haveNExt(sail_ctx, ()) && {
            let var_240 = bitvector_access(sail_ctx.sedeleg.bits, idx);
            bit_to_bool(sail_ctx, var_240)
        }))
    } else {
        (_super_ && haveNExt(sail_ctx, ()))
    };
    ...
    if {{
        let var_238 = privLevel_to_bits(sail_ctx, deleg);
        let var_239 = privLevel_to_bits(sail_ctx, p);
        _operator_smaller_u_(sail_ctx, var_238, var_239)
    }} {
        p
    } else {
        deleg
...
pub fn trap_handler(sail_ctx: &mut SailVirtCtx, del_priv: Privilege, intr: bool, c: BitVector<8>, pc: BitVector<64>, info: Option<BitVector<64>>, ext: Option<()>) -> BitVector<64> {
    ...
    match del_priv {
        Privilege::Machine => {{
            sail_ctx.mcause = {
                let var_242 = bool_to_bits(sail_ctx, intr);
                sail_ctx.mcause.set_subrange::<63, 64, 1>(var_242)
            };
            sail_ctx.mcause = {
                let var_243 = zero_extend_63(c);
                sail_ctx.mcause.set_subrange::<0, 63, 63>(var_243)
            };
            sail_ctx.mstatus = {
                let var_244 = _get_Mstatus_MIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<7, 8, 1>(var_244)
            };
            sail_ctx.mstatus = sail_ctx.mstatus.set_subrange::<3, 4, 1>(BitVector::<1>::new(0b0));
            sail_ctx.mstatus = {
                let var_245 = privLevel_to_bits(sail_ctx, sail_ctx.cur_privilege);
                sail_ctx.mstatus.set_subrange::<11, 13, 2>(var_245)
            };
        ...
        Privilege::Supervisor => {{
            ...
            sail_ctx.scause = {
                ...
                sail_ctx.scause.set_subrange::<63, 64, 1>(var_246)
            };
            sail_ctx.scause = {
                let var_247 = zero_extend_63(c);
                sail_ctx.scause.set_subrange::<0, 63, 63>(var_247)
            };
            sail_ctx.mstatus = {
                let var_248 = _get_Mstatus_SIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<5, 6, 1>(var_248)
            };
        ...
        Privilege::User => {{
            ...
            sail_ctx.ucause = {
                let var_249 = bool_to_bits(sail_ctx, intr);
                sail_ctx.ucause.set_subrange::<63, 64, 1>(var_249)
            };
            sail_ctx.ucause = {
                let var_250 = zero_extend_63(c);
                sail_ctx.ucause.set_subrange::<0, 63, 63>(var_250)
            };
            sail_ctx.mstatus = {
                let var_251 = _get_Mstatus_UIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<4, 5, 1>(var_251)
            };
...
pub fn exception_handler(sail_ctx: &mut SailVirtCtx, cur_priv: Privilege, ctl: ctl_result, pc: BitVector<64>) -> BitVector<64> {
    match (cur_priv, ctl) {
        (_, ctl_result::CTL_TRAP(e)) => {{
            let del_priv = {
                let var_260 = e.trap;
                let var_261 = cur_priv;
                exception_delegatee(sail_ctx, var_260, var_261)
            };
            if {get_config_print_platform(sail_ctx, ())} {
                print_platform(format!("{}{}", String::from("trapping from "), format!("{}{}", privLevel_to_str(sail_ctx, cur_priv), format!("{}{}", String::from(" to "), format!("{}{}", privLevel_to_str(sail_ctx, del_priv), format!("{}{}", String::from(" to handle "), {
                    let var_252 = e.trap;
                    exceptionType_to_str(sail_ctx, var_252)
                }))))))
            } else {
                ()
            };
            {
                let var_253 = del_priv;
                let var_254 = false;
                let var_255 = {
                    let var_259 = e.trap;
                    exceptionType_to_bits(sail_ctx, var_259)
                };
                let var_256 = pc;
                let var_257 = e.excinfo;
                let var_258 = e.ext;
                trap_handler(sail_ctx, var_253, var_254, var_255, var_256, var_257, var_258)
            }
        }}
        (_, ctl_result::CTL_MRET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_262 = _get_Mstatus_MPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<3, 4, 1>(var_262)
            };
            sail_ctx.mstatus = sail_ctx.mstatus.set_subrange::<7, 8, 1>(BitVector::<1>::new(0b1));
            sail_ctx.cur_privilege = {
                let var_263 = _get_Mstatus_MPP(sail_ctx, sail_ctx.mstatus);
                privLevel_of_bits(sail_ctx, var_263)
            };
            sail_ctx.mstatus = {
                let var_264 = {
                    let var_265 = if {haveUsrMode(sail_ctx, ())} {
                        Privilege::User
                    } else {
                        Privilege::Machine
                    };
                    privLevel_to_bits(sail_ctx, var_265)
                };
                sail_ctx.mstatus.set_subrange::<11, 13, 2>(var_264)
            };
        ...
        (_, ctl_result::CTL_SRET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_266 = _get_Mstatus_SPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<1, 2, 1>(var_266)
            };
        ...
        (_, ctl_result::CTL_URET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_267 = _get_Mstatus_UPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<0, 1, 1>(var_267)
            };
...
pub fn handle_illegal(sail_ctx: &mut SailVirtCtx, unit_arg: ()) {
    ...
    {
        let var_268 = {
            let var_269 = sail_ctx.cur_privilege;
            let var_270 = ctl_result::CTL_TRAP(t);
            let var_271 = sail_ctx.PC;
            exception_handler(sail_ctx, var_269, var_270, var_271)
        };
        set_next_pc(sail_ctx, var_268)
    }
}
...
pub fn readCSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>) -> BitVector<64> {
    let res: xlenbits = match (csr, 64) {
        ...
        (v__12, _) if {{
            ...
            let var_272 = idx.as_usize();
            pmpReadCfgReg(sail_ctx, var_272)}
        (v__14, _) if {(v__14.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111011))} => {let idx: BitVector<4> = v__14.subrange::<0, 4, 4>();
            let var_273 = bitvector_concat(BitVector::<2>::new(0b00), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_273)}
        (v__16, _) if {(v__16.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111100))} => {let idx: BitVector<4> = v__16.subrange::<0, 4, 4>();
            let var_274 = bitvector_concat(BitVector::<2>::new(0b01), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_274)}
        (v__18, _) if {(v__18.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111101))} => {let idx: BitVector<4> = v__18.subrange::<0, 4, 4>();
            let var_275 = bitvector_concat(BitVector::<2>::new(0b10), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_275)}
        (v__20, _) if {(v__20.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111110))} => {let idx: BitVector<4> = v__20.subrange::<0, 4, 4>();
            let var_276 = bitvector_concat(BitVector::<2>::new(0b11), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_276)}
...
pub fn writeCSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, value: BitVector<64>) {
    let res: Option<xlenbits> = match (csr, 64) {
        ...
        (b__28, _) if {(b__28 == BitVector::<12>::new(0b000100001010))} => {{
            sail_ctx.senvcfg = {
                let var_281 = sail_ctx.senvcfg;
                let var_282 = zero_extend_64(value);
                legalize_senvcfg(sail_ctx, var_281, var_282)
            };
        ...
        (b__34, _) if {(b__34 == BitVector::<12>::new(0b000110000000))} => {{
            sail_ctx.satp = {
                let var_283 = cur_Architecture(sail_ctx, ());
                let var_284 = sail_ctx.satp;
                let var_285 = value;
                legalize_satp(sail_ctx, var_283, var_284, var_285)
            };
            Some(sail_ctx.satp)
        }}
...
pub fn execute_ITYPE(sail_ctx: &mut SailVirtCtx, imm: BitVector<12>, rs1: regidx, rd: regidx, op: iop) -> Retired {
    ...
    let result: xlenbits = match op {
        ...
        iop::RISCV_SLTI => {zero_extend_64({
            let var_286 = _operator_smaller_s_(sail_ctx, rs1_val, immext);
            bool_to_bits(sail_ctx, var_286)
        })}
        iop::RISCV_SLTIU => {zero_extend_64({
            let var_287 = _operator_smaller_u_(sail_ctx, rs1_val, immext);
            bool_to_bits(sail_ctx, var_287)
        })}
...
pub fn execute_MRET(sail_ctx: &mut SailVirtCtx) -> Retired {
    if {(sail_ctx.cur_privilege != Privilege::Machine)} {
        ...
        {
            let var_288 = {
                let var_289 = sail_ctx.cur_privilege;
                let var_290 = ctl_result::CTL_MRET(());
                let var_291 = sail_ctx.PC;
                exception_handler(sail_ctx, var_289, var_290, var_291)
            };
            set_next_pc(sail_ctx, var_288)
        };
        Retired::RETIRE_SUCCESS
    }
}
