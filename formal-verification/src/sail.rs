...
pub fn hex_bits_forwards<const N: usize>(sail_ctx: &mut SailVirtCtx, bv: BitVector<N>) -> (usize, String) {
    (bitvector_length(bv), hex_str(bv.as_usize()))
}

pub fn hex_bits_forwards_matches<const N: usize>(sail_ctx: &mut SailVirtCtx, bv: BitVector<N>) -> bool {
    true
}
...
pub fn _operator_smallerequal_u_<const N: usize>(sail_ctx: &mut SailVirtCtx, x: BitVector<N>, y: BitVector<N>) -> bool {
...
pub fn privLevel_of_bits(sail_ctx: &mut SailVirtCtx, p: BitVector<2>) -> Privilege {
    match p {
        ...
        _ => {internal_error(String::from("../miralis-sail-riscv/model/riscv_types.sail"), 111, format!("{}{}", String::from("Invalid privilege level: "), bits_str(p)))}
    }
}
...
pub fn satp64Mode_of_bits(sail_ctx: &mut SailVirtCtx, a: Architecture, m: BitVector<4>) -> Option<SATPMode> {
    match (a, m) {
...
pub fn wX(sail_ctx: &mut SailVirtCtx, r: usize, in_v: BitVector<64>) {
    ...
    if {(r != 0)} {
        ...
        if {get_config_print_reg(sail_ctx, ())} {
            print_reg(format!("{}{}", String::from("x"), format!("{}{}", dec_str(r), format!("{}{}", String::from(" <- "), RegStr(sail_ctx, v)))))
        } else {
            ()
        }
    } else {
...
pub fn legalize_misa(sail_ctx: &mut SailVirtCtx, m: Misa, v: BitVector<64>) -> Misa {
    ...
    if {(!(sys_enable_writable_misa(())) || ((_get_Misa_C(sail_ctx, v) == BitVector::<1>::new(0b0)) && ((bitvector_access(sail_ctx.nextPC, 1) == true) || ext_veto_disable_C(sail_ctx, ()))))} {
        m
    } else {
...
pub fn legalize_mstatus(sail_ctx: &mut SailVirtCtx, o: Mstatus, v: BitVector<64>) -> Mstatus {
    let m: Mstatus = {
        let var_32 = zero_extend_64(bitvector_concat(v.subrange::<7, 23, 16>(), bitvector_concat(BitVector::<1>::new(0b0), bitvector_concat(v.subrange::<3, 6, 3>(), bitvector_concat(BitVector::<1>::new(0b0), v.subrange::<0, 2, 2>())))));
...
pub fn cur_Architecture(sail_ctx: &mut SailVirtCtx, unit_arg: ()) -> Architecture {
    ...
    match architecture(sail_ctx, a) {
        Some(a) => {a}
        None => {internal_error(String::from("../miralis-sail-riscv/model/riscv_sys_regs.sail"), 323, String::from("Invalid current architecture"))}
    }
}
...
pub fn _update_Minterrupts_MEI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
...
pub fn _update_Minterrupts_MSI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 3, 3, x)
    }
}
...
pub fn _update_Minterrupts_MTI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
...
pub fn _update_Minterrupts_SEI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
...
pub fn _update_Minterrupts_SSI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 1, 1, x)
    }
}
...
pub fn _update_Minterrupts_STI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 5, 5, x)
    }
}
...
pub fn _update_Minterrupts_UEI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 8, 8, x)
    }
}
...
pub fn _update_Minterrupts_USI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 0, 0, x)
    }
}
...
pub fn _update_Minterrupts_UTI(sail_ctx: &mut SailVirtCtx, v: Minterrupts, x: BitVector<1>) -> Minterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 4, 4, x)
    }
}
...
pub fn legalize_mideleg(sail_ctx: &mut SailVirtCtx, o: Minterrupts, v: BitVector<64>) -> Minterrupts {
    {
        let var_63 = {
            let var_65 = {
                let var_67 = {
                    let var_69 = {
                        let var_71 = {
                            let var_73 = Mk_Minterrupts(sail_ctx, v);
                            let var_74 = BitVector::<1>::new(0b0);
                            _update_Minterrupts_MEI(sail_ctx, var_73, var_74)
                        };
                        let var_72 = BitVector::<1>::new(0b0);
                        _update_Minterrupts_MTI(sail_ctx, var_71, var_72)
                    };
                    let var_70 = BitVector::<1>::new(0b0);
                    _update_Minterrupts_MSI(sail_ctx, var_69, var_70)
                };
                let var_68 = BitVector::<1>::new(0b1);
                _update_Minterrupts_SEI(sail_ctx, var_67, var_68)
            };
            let var_66 = BitVector::<1>::new(0b1);
            _update_Minterrupts_STI(sail_ctx, var_65, var_66)
        };
        let var_64 = BitVector::<1>::new(0b1);
        _update_Minterrupts_SSI(sail_ctx, var_63, var_64)
    }
}
...
pub fn _update_Medeleg_MEnvCall(sail_ctx: &mut SailVirtCtx, v: Medeleg, x: BitVector<1>) -> Medeleg {
    BitField {
        bits: update_subrange_bits(v.bits, 11, 11, x)
    }
}
...
pub fn legalize_medeleg(sail_ctx: &mut SailVirtCtx, o: Medeleg, v: BitVector<64>) -> Medeleg {
    {
        let var_75 = Mk_Medeleg(sail_ctx, v);
        let var_76 = BitVector::<1>::new(0b0);
        _update_Medeleg_MEnvCall(sail_ctx, var_75, var_76)
    }
}
...
pub fn legalize_tvec(sail_ctx: &mut SailVirtCtx, o: Mtvec, v: BitVector<64>) -> Mtvec {
    ...
    match {
        let var_79 = _get_Mtvec_Mode(sail_ctx, v);
        trapVectorMode_of_bits(sail_ctx, var_79)
    } {
        TrapVectorMode::TV_Direct => {v}
        TrapVectorMode::TV_Vector => {v}
        _ => {{
            let var_77 = v;
            let var_78 = _get_Mtvec_Mode(sail_ctx, o);
            _update_Mtvec_Mode(sail_ctx, var_77, var_78)
        }}
    }
}
...
pub fn tvec_addr(sail_ctx: &mut SailVirtCtx, m: Mtvec, c: Mcause) -> Option<BitVector<64>> {
    let base: xlenbits = bitvector_concat::<62,2>(_get_Mtvec_Base(sail_ctx, m), BitVector::<2>::new(0b00));
    match {
        let var_81 = _get_Mtvec_Mode(sail_ctx, m);
        trapVectorMode_of_bits(sail_ctx, var_81)
    } {
        TrapVectorMode::TV_Direct => {Some(base)}
        TrapVectorMode::TV_Vector => {if {(_get_Mcause_IsInterrupt(sail_ctx, c) == BitVector::<1>::new(0b1))} {
            Some({
                let var_80 = (zero_extend_64(_get_Mcause_Cause(sail_ctx, c)) << 2);
                base.wrapped_add(var_80)
            })
        } else {
            Some(base)
        }}
...
pub fn legalize_xepc(sail_ctx: &mut SailVirtCtx, v: BitVector<64>) -> BitVector<64> {
    if {((sys_enable_writable_misa(()) && sys_enable_rvc(())) || (_get_Misa_C(sail_ctx, sail_ctx.misa) == BitVector::<1>::new(0b1)))} {
...
pub fn pc_alignment_mask(sail_ctx: &mut SailVirtCtx, unit_arg: ()) -> BitVector<64> {
    !(zero_extend_64(if {(_get_Misa_C(sail_ctx, sail_ctx.misa) == BitVector::<1>::new(0b1))} {
...
pub fn _update_Counteren_CY(sail_ctx: &mut SailVirtCtx, v: Counteren, x: BitVector<1>) -> Counteren {
    BitField {
        bits: update_subrange_bits(v.bits, 0, 0, x)
    }
}
...
pub fn _update_Counteren_IR(sail_ctx: &mut SailVirtCtx, v: Counteren, x: BitVector<1>) -> Counteren {
    BitField {
        bits: update_subrange_bits(v.bits, 2, 2, x)
    }
}
...
pub fn _update_Counteren_TM(sail_ctx: &mut SailVirtCtx, v: Counteren, x: BitVector<1>) -> Counteren {
    BitField {
        bits: update_subrange_bits(v.bits, 1, 1, x)
    }
}

pub fn legalize_mcounteren(sail_ctx: &mut SailVirtCtx, c: Counteren, v: BitVector<64>) -> Counteren {
    {
        let var_82 = {
            let var_86 = _update_Counteren_IR(sail_ctx, c, {
                let mut __generated_vector: BitVector<1> = BitVector::<1>::new_empty();
                {
                    let var_90 = 0;
                    let var_91 = bitvector_access(v, 2);
                    __generated_vector.set_vector_entry(var_90, var_91)
                };
                __generated_vector
            });
            let var_87 = {
                ...
                {
                    let var_88 = 0;
                    let var_89 = bitvector_access(v, 1);
                    __generated_vector.set_vector_entry(var_88, var_89)
                };
                __generated_vector
            };
            _update_Counteren_TM(sail_ctx, var_86, var_87)
        };
        let var_83 = {
            let mut __generated_vector: BitVector<1> = BitVector::<1>::new_empty();
            {
                let var_84 = 0;
                let var_85 = bitvector_access(v, 0);
                __generated_vector.set_vector_entry(var_84, var_85)
            };
            __generated_vector
        };
        _update_Counteren_CY(sail_ctx, var_82, var_83)
    }
}

pub fn legalize_scounteren(sail_ctx: &mut SailVirtCtx, c: Counteren, v: BitVector<64>) -> Counteren {
    {
        let var_92 = {
            let var_96 = _update_Counteren_IR(sail_ctx, c, {
                let mut __generated_vector: BitVector<1> = BitVector::<1>::new_empty();
                {
                    let var_100 = 0;
            ...
            let var_97 = {
                ...
                {
                    let var_98 = 0;
                    let var_99 = bitvector_access(v, 1);
                    __generated_vector.set_vector_entry(var_98, var_99)
                };
                __generated_vector
            };
            _update_Counteren_TM(sail_ctx, var_96, var_97)
        };
        let var_93 = {
            let mut __generated_vector: BitVector<1> = BitVector::<1>::new_empty();
            {
                let var_94 = 0;
                let var_95 = bitvector_access(v, 0);
                __generated_vector.set_vector_entry(var_94, var_95)
            };
            __generated_vector
        };
        _update_Counteren_CY(sail_ctx, var_92, var_93)
    }
}

pub fn _update_Counterin_CY(sail_ctx: &mut SailVirtCtx, v: Counterin, x: BitVector<1>) -> Counterin {
    BitField {
        bits: update_subrange_bits(v.bits, 0, 0, x)
    }
}

pub fn _update_Counterin_IR(sail_ctx: &mut SailVirtCtx, v: Counterin, x: BitVector<1>) -> Counterin {
    BitField {
        bits: update_subrange_bits(v.bits, 2, 2, x)
    }
}

pub fn legalize_mcountinhibit(sail_ctx: &mut SailVirtCtx, c: Counterin, v: BitVector<64>) -> Counterin {
    {
        let var_102 = _update_Counterin_IR(sail_ctx, c, {
            let mut __generated_vector: BitVector<1> = BitVector::<1>::new_empty();
            {
                let var_106 = 0;
                let var_107 = bitvector_access(v, 2);
                __generated_vector.set_vector_entry(var_106, var_107)
            };
            __generated_vector
        });
        let var_103 = {
            ...
            {
                let var_104 = 0;
                let var_105 = bitvector_access(v, 0);
                __generated_vector.set_vector_entry(var_104, var_105)
            };
            __generated_vector
        };
        _update_Counterin_CY(sail_ctx, var_102, var_103)
    }
}
...
pub fn set_sstatus_UXL(sail_ctx: &mut SailVirtCtx, s: Sstatus, a: BitVector<2>) -> Sstatus {
    let m = {
        let var_109 = s.bits;
        Mk_Mstatus(sail_ctx, var_109)
    };
    let m = set_mstatus_UXL(sail_ctx, m, a);
    {
        let var_108 = m.bits;
        Mk_Sstatus(sail_ctx, var_108)
    }
}

pub fn lower_mstatus(sail_ctx: &mut SailVirtCtx, m: Mstatus) -> Sstatus {
    let s = {
        let var_134 = zero_extend_64(BitVector::<1>::new(0b0));
        Mk_Sstatus(sail_ctx, var_134)
    };
    let s = {
        let var_132 = s;
        let var_133 = _get_Mstatus_SD(sail_ctx, m);
        _update_Sstatus_SD(sail_ctx, var_132, var_133)
    };
    let s = {
        let var_130 = s;
        let var_131 = get_mstatus_UXL(sail_ctx, m);
        set_sstatus_UXL(sail_ctx, var_130, var_131)
    };
    let s = {
        let var_128 = s;
        let var_129 = _get_Mstatus_MXR(sail_ctx, m);
        _update_Sstatus_MXR(sail_ctx, var_128, var_129)
    };
    let s = {
        let var_126 = s;
        let var_127 = _get_Mstatus_SUM(sail_ctx, m);
        _update_Sstatus_SUM(sail_ctx, var_126, var_127)
    };
    let s = {
        let var_124 = s;
        let var_125 = _get_Mstatus_XS(sail_ctx, m);
        _update_Sstatus_XS(sail_ctx, var_124, var_125)
    };
    let s = {
        let var_122 = s;
        let var_123 = _get_Mstatus_FS(sail_ctx, m);
        _update_Sstatus_FS(sail_ctx, var_122, var_123)
    };
    let s = {
        let var_120 = s;
        let var_121 = _get_Mstatus_VS(sail_ctx, m);
        _update_Sstatus_VS(sail_ctx, var_120, var_121)
    };
    let s = {
        let var_118 = s;
        let var_119 = _get_Mstatus_SPP(sail_ctx, m);
        _update_Sstatus_SPP(sail_ctx, var_118, var_119)
    };
    let s = {
        let var_116 = s;
        let var_117 = _get_Mstatus_SPIE(sail_ctx, m);
        _update_Sstatus_SPIE(sail_ctx, var_116, var_117)
    };
    let s = {
        let var_114 = s;
        let var_115 = _get_Mstatus_UPIE(sail_ctx, m);
        _update_Sstatus_UPIE(sail_ctx, var_114, var_115)
    };
    let s = {
        let var_112 = s;
        let var_113 = _get_Mstatus_SIE(sail_ctx, m);
        _update_Sstatus_SIE(sail_ctx, var_112, var_113)
    };
    let s = {
        let var_110 = s;
        let var_111 = _get_Mstatus_UIE(sail_ctx, m);
        _update_Sstatus_UIE(sail_ctx, var_110, var_111)
    };
    s
}
...
pub fn lift_sstatus(sail_ctx: &mut SailVirtCtx, m: Mstatus, s: Sstatus) -> Mstatus {
    let m = {
        let var_158 = m;
        let var_159 = _get_Sstatus_MXR(sail_ctx, s);
        _update_Mstatus_MXR(sail_ctx, var_158, var_159)
    };
    let m = {
        let var_156 = m;
        let var_157 = _get_Sstatus_SUM(sail_ctx, s);
        _update_Mstatus_SUM(sail_ctx, var_156, var_157)
    };
    let m = {
        let var_154 = m;
        let var_155 = _get_Sstatus_XS(sail_ctx, s);
        _update_Mstatus_XS(sail_ctx, var_154, var_155)
    };
    let m = {
        let var_152 = m;
        let var_153 = _get_Sstatus_FS(sail_ctx, s);
        _update_Mstatus_FS(sail_ctx, var_152, var_153)
    };
    let m = {
        let var_150 = m;
        let var_151 = _get_Sstatus_VS(sail_ctx, s);
        _update_Mstatus_VS(sail_ctx, var_150, var_151)
    };
    let dirty = (({
        let var_149 = _get_Mstatus_FS(sail_ctx, m);
        extStatus_of_bits(sail_ctx, var_149)
    } == ExtStatus::Dirty) || (({
        let var_148 = _get_Mstatus_XS(sail_ctx, m);
        extStatus_of_bits(sail_ctx, var_148)
    } == ExtStatus::Dirty) || ({
        let var_147 = _get_Mstatus_VS(sail_ctx, m);
        extStatus_of_bits(sail_ctx, var_147)
    } == ExtStatus::Dirty)));
    let m = {
        let var_145 = m;
        let var_146 = bool_to_bits(sail_ctx, dirty);
        _update_Mstatus_SD(sail_ctx, var_145, var_146)
    };
    let m = {
        let var_143 = m;
        let var_144 = _get_Sstatus_SPP(sail_ctx, s);
        _update_Mstatus_SPP(sail_ctx, var_143, var_144)
    };
    let m = {
        let var_141 = m;
        let var_142 = _get_Sstatus_SPIE(sail_ctx, s);
        _update_Mstatus_SPIE(sail_ctx, var_141, var_142)
    };
    let m = {
        let var_139 = m;
        let var_140 = _get_Sstatus_UPIE(sail_ctx, s);
        _update_Mstatus_UPIE(sail_ctx, var_139, var_140)
    };
    let m = {
        let var_137 = m;
        let var_138 = _get_Sstatus_SIE(sail_ctx, s);
        _update_Mstatus_SIE(sail_ctx, var_137, var_138)
    };
    let m = {
        let var_135 = m;
        let var_136 = _get_Sstatus_UIE(sail_ctx, s);
        _update_Mstatus_UIE(sail_ctx, var_135, var_136)
    };
    m
}
...
pub fn legalize_sstatus(sail_ctx: &mut SailVirtCtx, m: Mstatus, v: BitVector<64>) -> Mstatus {
    {
        ...
        let var_161 = {
            let var_162 = m;
            let var_163 = Mk_Sstatus(sail_ctx, v);
            lift_sstatus(sail_ctx, var_162, var_163)
        }.bits;
        legalize_mstatus(sail_ctx, var_160, var_161)
    }
}
...
pub fn legalize_sedeleg(sail_ctx: &mut SailVirtCtx, s: Sedeleg, v: BitVector<64>) -> Sedeleg {
    {
        let var_164 = zero_extend_64(v.subrange::<0, 9, 9>());
        Mk_Sedeleg(sail_ctx, var_164)
    }
}
...
pub fn _update_Sinterrupts_SEI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 9, 9, x)
    }
}
...
pub fn _update_Sinterrupts_SSI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 1, 1, x)
    }
}
...
pub fn _update_Sinterrupts_STI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 5, 5, x)
    }
}
...
pub fn _update_Sinterrupts_UEI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 8, 8, x)
    }
}
...
pub fn _update_Sinterrupts_USI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 0, 0, x)
    }
}
...
pub fn _update_Sinterrupts_UTI(sail_ctx: &mut SailVirtCtx, v: Sinterrupts, x: BitVector<1>) -> Sinterrupts {
    BitField {
        bits: update_subrange_bits(v.bits, 4, 4, x)
    }
}
...
pub fn lower_mip(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts) -> Sinterrupts {
    let s: Sinterrupts = {
        let var_177 = zero_extend_64(BitVector::<1>::new(0b0));
        Mk_Sinterrupts(sail_ctx, var_177)
    };
    let s = {
        let var_175 = s;
        let var_176 = (_get_Minterrupts_SEI(sail_ctx, m) & _get_Minterrupts_SEI(sail_ctx, d));
        _update_Sinterrupts_SEI(sail_ctx, var_175, var_176)
    };
    let s = {
        let var_173 = s;
        let var_174 = (_get_Minterrupts_STI(sail_ctx, m) & _get_Minterrupts_STI(sail_ctx, d));
        _update_Sinterrupts_STI(sail_ctx, var_173, var_174)
    };
    let s = {
        let var_171 = s;
        let var_172 = (_get_Minterrupts_SSI(sail_ctx, m) & _get_Minterrupts_SSI(sail_ctx, d));
        _update_Sinterrupts_SSI(sail_ctx, var_171, var_172)
    };
    let s = {
        let var_169 = s;
        let var_170 = (_get_Minterrupts_UEI(sail_ctx, m) & _get_Minterrupts_UEI(sail_ctx, d));
        _update_Sinterrupts_UEI(sail_ctx, var_169, var_170)
    };
    let s = {
        let var_167 = s;
        let var_168 = (_get_Minterrupts_UTI(sail_ctx, m) & _get_Minterrupts_UTI(sail_ctx, d));
        _update_Sinterrupts_UTI(sail_ctx, var_167, var_168)
    };
    let s = {
        let var_165 = s;
        let var_166 = (_get_Minterrupts_USI(sail_ctx, m) & _get_Minterrupts_USI(sail_ctx, d));
        _update_Sinterrupts_USI(sail_ctx, var_165, var_166)
    };
    s
}
...
pub fn lower_mie(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts) -> Sinterrupts {
    let s: Sinterrupts = {
        let var_190 = zero_extend_64(BitVector::<1>::new(0b0));
        Mk_Sinterrupts(sail_ctx, var_190)
    };
    let s = {
        let var_188 = s;
        let var_189 = (_get_Minterrupts_SEI(sail_ctx, m) & _get_Minterrupts_SEI(sail_ctx, d));
        _update_Sinterrupts_SEI(sail_ctx, var_188, var_189)
    };
    let s = {
        let var_186 = s;
        let var_187 = (_get_Minterrupts_STI(sail_ctx, m) & _get_Minterrupts_STI(sail_ctx, d));
        _update_Sinterrupts_STI(sail_ctx, var_186, var_187)
    };
    let s = {
        let var_184 = s;
        let var_185 = (_get_Minterrupts_SSI(sail_ctx, m) & _get_Minterrupts_SSI(sail_ctx, d));
        _update_Sinterrupts_SSI(sail_ctx, var_184, var_185)
    };
    let s = {
        let var_182 = s;
        let var_183 = (_get_Minterrupts_UEI(sail_ctx, m) & _get_Minterrupts_UEI(sail_ctx, d));
        _update_Sinterrupts_UEI(sail_ctx, var_182, var_183)
    };
    let s = {
        let var_180 = s;
        let var_181 = (_get_Minterrupts_UTI(sail_ctx, m) & _get_Minterrupts_UTI(sail_ctx, d));
        _update_Sinterrupts_UTI(sail_ctx, var_180, var_181)
    };
    let s = {
        let var_178 = s;
        let var_179 = (_get_Minterrupts_USI(sail_ctx, m) & _get_Minterrupts_USI(sail_ctx, d));
        _update_Sinterrupts_USI(sail_ctx, var_178, var_179)
    };
    s
}

pub fn lift_sip(sail_ctx: &mut SailVirtCtx, o: Minterrupts, d: Minterrupts, s: Sinterrupts) -> Minterrupts {
    let m: Minterrupts = o;
    let m = if {(_get_Minterrupts_SSI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_195 = m;
            let var_196 = _get_Sinterrupts_SSI(sail_ctx, s);
            _update_Minterrupts_SSI(sail_ctx, var_195, var_196)
        }
    } else {
        m
    };
    if {haveNExt(sail_ctx, ())} {
        let m = if {(_get_Minterrupts_UEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_193 = m;
                let var_194 = _get_Sinterrupts_UEI(sail_ctx, s);
                _update_Minterrupts_UEI(sail_ctx, var_193, var_194)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_USI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_191 = m;
                let var_192 = _get_Sinterrupts_USI(sail_ctx, s);
                _update_Minterrupts_USI(sail_ctx, var_191, var_192)
            }
        } else {
            m
        };
...
pub fn legalize_sip(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts, v: BitVector<64>) -> Minterrupts {
    {
        let var_197 = m;
        let var_198 = d;
        let var_199 = Mk_Sinterrupts(sail_ctx, v);
        lift_sip(sail_ctx, var_197, var_198, var_199)
    }
}

pub fn lift_sie(sail_ctx: &mut SailVirtCtx, o: Minterrupts, d: Minterrupts, s: Sinterrupts) -> Minterrupts {
    let m: Minterrupts = o;
    let m = if {(_get_Minterrupts_SEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_210 = m;
            let var_211 = _get_Sinterrupts_SEI(sail_ctx, s);
            _update_Minterrupts_SEI(sail_ctx, var_210, var_211)
        }
    } else {
        m
    };
    let m = if {(_get_Minterrupts_STI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_208 = m;
            let var_209 = _get_Sinterrupts_STI(sail_ctx, s);
            _update_Minterrupts_STI(sail_ctx, var_208, var_209)
        }
    } else {
        m
    };
    let m = if {(_get_Minterrupts_SSI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
        {
            let var_206 = m;
            let var_207 = _get_Sinterrupts_SSI(sail_ctx, s);
            _update_Minterrupts_SSI(sail_ctx, var_206, var_207)
        }
    } else {
        m
    };
    if {haveNExt(sail_ctx, ())} {
        let m = if {(_get_Minterrupts_UEI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_204 = m;
                let var_205 = _get_Sinterrupts_UEI(sail_ctx, s);
                _update_Minterrupts_UEI(sail_ctx, var_204, var_205)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_UTI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_202 = m;
                let var_203 = _get_Sinterrupts_UTI(sail_ctx, s);
                _update_Minterrupts_UTI(sail_ctx, var_202, var_203)
            }
        } else {
            m
        };
        let m = if {(_get_Minterrupts_USI(sail_ctx, d) == BitVector::<1>::new(0b1))} {
            {
                let var_200 = m;
                let var_201 = _get_Sinterrupts_USI(sail_ctx, s);
                _update_Minterrupts_USI(sail_ctx, var_200, var_201)
            }
        } else {
            m
        };
...
pub fn legalize_sie(sail_ctx: &mut SailVirtCtx, m: Minterrupts, d: Minterrupts, v: BitVector<64>) -> Minterrupts {
    {
        let var_212 = m;
        let var_213 = d;
        let var_214 = Mk_Sinterrupts(sail_ctx, v);
        lift_sie(sail_ctx, var_212, var_213, var_214)
    }
}
...
pub fn legalize_satp64(sail_ctx: &mut SailVirtCtx, a: Architecture, o: BitVector<64>, v: BitVector<64>) -> BitVector<64> {
    let s = Mk_Satp64(sail_ctx, v);
    match {
        let var_215 = a;
        let var_216 = _get_Satp64_Mode(sail_ctx, s);
        satp64Mode_of_bits(sail_ctx, var_215, var_216)
    } {
        None => {o}
...
pub fn legalize_satp32(sail_ctx: &mut SailVirtCtx, a: Architecture, o: BitVector<32>, v: BitVector<32>) -> BitVector<32> {
    v
}
...
pub fn read_seed_csr(sail_ctx: &mut SailVirtCtx, unit_arg: ()) -> BitVector<64> {
    ...
    zero_extend_64(bitvector_concat(opst_code_forwards(sail_ctx, seed_opst::ES16), bitvector_concat(reserved_bits, bitvector_concat(custom_bits, seed))))
}
...
pub fn legalize_menvcfg(sail_ctx: &mut SailVirtCtx, o: MEnvcfg, v: BitVector<64>) -> MEnvcfg {
    ...
    let o = {
        let var_217 = o;
        let var_218 = if {sys_enable_writable_fiom(())} {
            _get_MEnvcfg_FIOM(sail_ctx, v)
        } else {
            BitVector::<1>::new(0b0)
        };
        _update_MEnvcfg_FIOM(sail_ctx, var_217, var_218)
    };
    o
}
...
pub fn legalize_senvcfg(sail_ctx: &mut SailVirtCtx, o: SEnvcfg, v: BitVector<64>) -> SEnvcfg {
    ...
    let o = {
        let var_219 = o;
        let var_220 = if {sys_enable_writable_fiom(())} {
            _get_SEnvcfg_FIOM(sail_ctx, v)
        } else {
            BitVector::<1>::new(0b0)
        };
        _update_SEnvcfg_FIOM(sail_ctx, var_219, var_220)
    };
    o
}
...
pub fn _update_Pmpcfg_ent_A(sail_ctx: &mut SailVirtCtx, v: Pmpcfg_ent, x: BitVector<2>) -> Pmpcfg_ent {
    BitField {
...
pub fn _update_Pmpcfg_ent_R(sail_ctx: &mut SailVirtCtx, v: Pmpcfg_ent, x: BitVector<1>) -> Pmpcfg_ent {
    BitField {
        bits: update_subrange_bits(v.bits, 0, 0, x)
    }
}
...
pub fn _update_Pmpcfg_ent_W(sail_ctx: &mut SailVirtCtx, v: Pmpcfg_ent, x: BitVector<1>) -> Pmpcfg_ent {
    BitField {
        bits: update_subrange_bits(v.bits, 1, 1, x)
    }
}

pub fn _update_Pmpcfg_ent_X(sail_ctx: &mut SailVirtCtx, v: Pmpcfg_ent, x: BitVector<1>) -> Pmpcfg_ent {
    BitField {
        bits: update_subrange_bits(v.bits, 2, 2, x)
    }
}
...
pub fn pmpReadCfgReg(sail_ctx: &mut SailVirtCtx, n: usize) -> BitVector<64> {
    if {(64 == 32)} {
        ...
        bitvector_concat::<8, 56>(            sail_ctx.pmpcfg_n[((n * 4) + 7)].bits,            bitvector_concat::<8, 48>(                sail_ctx.pmpcfg_n[((n * 4) + 6)].bits,                bitvector_concat::<8, 40>(                    sail_ctx.pmpcfg_n[((n * 4) + 5)].bits,                    bitvector_concat::<8, 32>(                        sail_ctx.pmpcfg_n[((n * 4) + 4)].bits,                        bitvector_concat::<8, 24>(                            sail_ctx.pmpcfg_n[((n * 4) + 3)].bits,                            bitvector_concat::<8, 16>(                                sail_ctx.pmpcfg_n[((n * 4) + 2)].bits,                                bitvector_concat::<8, 8>(                                    sail_ctx.pmpcfg_n[((n * 4) + 1)].bits,                                    sail_ctx.pmpcfg_n[((n * 4) + 0)].bits,                                ),                            ),                        ),                    ),                ),            ),        )
    }
}
    ...
    match bitvector_access(match_type, 1) {
        true if {(G >= 2)} => {{
            let mask: xlenbits = zero_extend_64({
                let var_221 = min_int((G - 1), 64);
                ones::<64>(sail_ctx, var_221)
            });
            (addr | mask)
        }}
        false if {(G >= 1)} => {{
            let mask: xlenbits = zero_extend_64({
                let var_222 = min_int(G, 64);
                ones::<64>(sail_ctx, var_222)
            });
            (addr & !(mask))
        }}
        _ => {addr}
...
pub fn pmpTORLocked(sail_ctx: &mut SailVirtCtx, cfg: Pmpcfg_ent) -> bool {
    ((_get_Pmpcfg_ent_L(sail_ctx, cfg) == BitVector::<1>::new(0b1)) && ({
        let var_223 = _get_Pmpcfg_ent_A(sail_ctx, cfg);
        pmpAddrMatchType_of_bits(sail_ctx, var_223)
...
pub fn pmpWriteCfg(sail_ctx: &mut SailVirtCtx, n: usize, cfg: Pmpcfg_ent, v: BitVector<8>) -> Pmpcfg_ent {
    if {pmpLocked(sail_ctx, cfg)} {
        ...
        let cfg = {
            let var_231 = (v & BitVector::<8>::new(0b10011111));
            Mk_Pmpcfg_ent(sail_ctx, var_231)
        };
        let cfg = if {((_get_Pmpcfg_ent_W(sail_ctx, cfg) == BitVector::<1>::new(0b1)) && (_get_Pmpcfg_ent_R(sail_ctx, cfg) == BitVector::<1>::new(0b0)))} {
            {
                let var_227 = {
                    let var_229 = _update_Pmpcfg_ent_X(sail_ctx, cfg, BitVector::<1>::new(0b0));
                    let var_230 = BitVector::<1>::new(0b0);
                    _update_Pmpcfg_ent_W(sail_ctx, var_229, var_230)
                };
                let var_228 = BitVector::<1>::new(0b0);
                _update_Pmpcfg_ent_R(sail_ctx, var_227, var_228)
            }
        } else {
            cfg
        };
        let cfg = if {((sys_pmp_grain(()) >= 1) && ({
            let var_226 = _get_Pmpcfg_ent_A(sail_ctx, cfg);
            pmpAddrMatchType_of_bits(sail_ctx, var_226)
        } == PmpAddrMatchType::NA4))} {
            {
                let var_224 = cfg;
                let var_225 = pmpAddrMatchType_to_bits(sail_ctx, PmpAddrMatchType::OFF);
                _update_Pmpcfg_ent_A(sail_ctx, var_224, var_225)
            }
        } else {
            cfg
        };
    ...
    if {(64 == 32)} {
        ...
        for i in 0..=7 {
            ...
            sail_ctx.pmpcfg_n[idx] = {
                let var_232 = idx;
                let var_233 = sail_ctx.pmpcfg_n[idx];
                let var_234 = subrange_bits_8(v, ((8 * i) + 7), (8 * i));
                pmpWriteCfg(sail_ctx, var_232, var_233, var_234)
            }
        }
    }
}

pub fn pmpWriteAddr(sail_ctx: &mut SailVirtCtx, locked: bool, tor_locked: bool, reg: BitVector<64>, v: BitVector<64>) -> BitVector<64> {
    if {(64 == 32)} {
        panic!("unreachable code")
    } else {
...
pub fn pmpWriteAddrReg(sail_ctx: &mut SailVirtCtx, n: usize, v: BitVector<64>) {
    sail_ctx.pmpaddr_n[n] = {
        let var_238 = pmpLocked(sail_ctx, sail_ctx.pmpcfg_n[n]);
        let var_239 = if {((n + 1) < 64)} {
            pmpTORLocked(sail_ctx, sail_ctx.pmpcfg_n[(n + 1)])
        } else {
            false
        };
        let var_240 = sail_ctx.pmpaddr_n[n];
        let var_241 = v;
        pmpWriteAddr(sail_ctx, var_238, var_239, var_240, var_241)
    }
}
...
pub fn ext_check_CSR(sail_ctx: &mut SailVirtCtx, csrno: BitVector<12>, p: Privilege, isWrite: bool) -> bool {
    true
}
...
pub fn handle_trap_extension(sail_ctx: &mut SailVirtCtx, p: Privilege, pc: BitVector<64>, u: Option<()>) {
    ()
}
...
pub fn prepare_trap_vector(sail_ctx: &mut SailVirtCtx, p: Privilege, cause: Mcause) -> BitVector<64> {
    ...
    match tvec_addr(sail_ctx, tvec, cause) {
        Some(epc) => {epc}
        None => {internal_error(String::from("../miralis-sail-riscv/model/riscv_sys_exceptions.sail"), 29, String::from("Invalid tvec mode"))}
    }
}
...
pub fn set_xret_target(sail_ctx: &mut SailVirtCtx, p: Privilege, value: BitVector<64>) -> BitVector<64> {
...
pub fn csr_name_map_forwards(sail_ctx: &mut SailVirtCtx, arg_hashtag_: BitVector<12>) -> String {
    match arg_hashtag_ {
        ...
        b__43 if {(b__43 == BitVector::<12>::new(0b001100100000))} => {String::from("mcountinhibit")}
...
pub fn ext_write_CSR(sail_ctx: &mut SailVirtCtx, _: BitVector<12>, _: BitVector<64>) -> Option<BitVector<64>> {
    None
}
...
pub fn is_CSR_defined(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege) -> bool {
    match csr {
        ...
        b__7 if {(b__7 == BitVector::<12>::new(0b001100000010))} => {((p == Privilege::Machine) && (haveSupMode(sail_ctx, ()) || haveNExt(sail_ctx, ())))}
        b__8 if {(b__8 == BitVector::<12>::new(0b001100000011))} => {((p == Privilege::Machine) && (haveSupMode(sail_ctx, ()) || haveNExt(sail_ctx, ())))}
        ...
        b__11 if {(b__11 == BitVector::<12>::new(0b001100000110))} => {((p == Privilege::Machine) && haveUsrMode(sail_ctx, ()))}
        b__12 if {(b__12 == BitVector::<12>::new(0b001100001010))} => {((p == Privilege::Machine) && haveUsrMode(sail_ctx, ()))}
        b__13 if {(b__13 == BitVector::<12>::new(0b001100010000))} => {((p == Privilege::Machine) && (64 == 32))}
        b__14 if {(b__14 == BitVector::<12>::new(0b001100011010))} => {((p == Privilege::Machine) && (haveUsrMode(sail_ctx, ()) && (64 == 32)))}
        ...
        v__0 if {(v__0.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111010))} => {let idx: BitVector<4> = v__0.subrange::<0, 4, 4>();
            ((p == Privilege::Machine) && ((gt_int(sys_pmp_count(()), idx.as_usize()) && ((bitvector_access(idx, 0) == false) || (64 == 32))) as bool))}
        v__2 if {(v__2.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111011))} => {let idx: BitVector<4> = v__2.subrange::<0, 4, 4>();
            ((p == Privilege::Machine) && (gt_int(sys_pmp_count(()), bitvector_concat(BitVector::<2>::new(0b00), idx).as_usize()) as bool))}
        v__4 if {(v__4.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111100))} => {let idx: BitVector<4> = v__4.subrange::<0, 4, 4>();
            ((p == Privilege::Machine) && (gt_int(sys_pmp_count(()), bitvector_concat(BitVector::<2>::new(0b01), idx).as_usize()) as bool))}
        v__6 if {(v__6.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111101))} => {let idx: BitVector<4> = v__6.subrange::<0, 4, 4>();
            ((p == Privilege::Machine) && (gt_int(sys_pmp_count(()), bitvector_concat(BitVector::<2>::new(0b10), idx).as_usize()) as bool))}
        v__8 if {(v__8.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111110))} => {let idx: BitVector<4> = v__8.subrange::<0, 4, 4>();
            ((p == Privilege::Machine) && (gt_int(sys_pmp_count(()), bitvector_concat(BitVector::<2>::new(0b11), idx).as_usize()) as bool))}
        ...
        b__23 if {(b__23 == BitVector::<12>::new(0b101110000000))} => {((p == Privilege::Machine) && (64 == 32))}
        b__24 if {(b__24 == BitVector::<12>::new(0b101110000010))} => {((p == Privilege::Machine) && (64 == 32))}
        b__25 if {(b__25 == BitVector::<12>::new(0b011110100000))} => {(p == Privilege::Machine)}
        b__26 if {(b__26 == BitVector::<12>::new(0b000100000000))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__27 if {(b__27 == BitVector::<12>::new(0b000100000010))} => {(haveSupMode(sail_ctx, ()) && (haveNExt(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor))))}
        b__28 if {(b__28 == BitVector::<12>::new(0b000100000011))} => {(haveSupMode(sail_ctx, ()) && (haveNExt(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor))))}
        b__29 if {(b__29 == BitVector::<12>::new(0b000100000100))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__30 if {(b__30 == BitVector::<12>::new(0b000100000101))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__31 if {(b__31 == BitVector::<12>::new(0b000100000110))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__32 if {(b__32 == BitVector::<12>::new(0b000100001010))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__33 if {(b__33 == BitVector::<12>::new(0b000101000000))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__34 if {(b__34 == BitVector::<12>::new(0b000101000001))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__35 if {(b__35 == BitVector::<12>::new(0b000101000010))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__36 if {(b__36 == BitVector::<12>::new(0b000101000011))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__37 if {(b__37 == BitVector::<12>::new(0b000101000100))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        b__38 if {(b__38 == BitVector::<12>::new(0b000110000000))} => {(haveSupMode(sail_ctx, ()) && ((p == Privilege::Machine) || (p == Privilege::Supervisor)))}
        ...
        b__42 if {(b__42 == BitVector::<12>::new(0b110010000000))} => {(haveUsrMode(sail_ctx, ()) && (64 == 32))}
        b__43 if {(b__43 == BitVector::<12>::new(0b110010000001))} => {(haveUsrMode(sail_ctx, ()) && (64 == 32))}
        b__44 if {(b__44 == BitVector::<12>::new(0b110010000010))} => {(haveUsrMode(sail_ctx, ()) && (64 == 32))}
...
pub fn check_CSR_access(sail_ctx: &mut SailVirtCtx, csrrw: BitVector<2>, csrpr: BitVector<2>, p: Privilege, isWrite: bool) -> bool {
    (!(((isWrite == true) && (csrrw == BitVector::<2>::new(0b11)))) && {
        let var_242 = privLevel_to_bits(sail_ctx, p);
        let var_243 = csrpr;
        _operator_biggerequal_u_(sail_ctx, var_242, var_243)
    })
}

pub fn check_TVM_SATP(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege) -> bool {
    !(((csr == BitVector::<12>::new(0b000110000000)) && ((p == Privilege::Supervisor) && (_get_Mstatus_TVM(sail_ctx, sail_ctx.mstatus) == BitVector::<1>::new(0b1)))))
}

pub fn check_Counteren(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege) -> bool {
    match (csr, p) {
        (b__0, Privilege::Supervisor) if {(b__0 == BitVector::<12>::new(0b110000000000))} => {(_get_Counteren_CY(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1))}
        (b__1, Privilege::Supervisor) if {(b__1 == BitVector::<12>::new(0b110000000001))} => {(_get_Counteren_TM(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1))}
        (b__2, Privilege::Supervisor) if {(b__2 == BitVector::<12>::new(0b110000000010))} => {(_get_Counteren_IR(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1))}
        (b__3, Privilege::User) if {(b__3 == BitVector::<12>::new(0b110000000000))} => {((_get_Counteren_CY(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1)) && (!(haveSupMode(sail_ctx, ())) || (_get_Counteren_CY(sail_ctx, sail_ctx.scounteren) == BitVector::<1>::new(0b1))))}
        (b__4, Privilege::User) if {(b__4 == BitVector::<12>::new(0b110000000001))} => {((_get_Counteren_TM(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1)) && (!(haveSupMode(sail_ctx, ())) || (_get_Counteren_TM(sail_ctx, sail_ctx.scounteren) == BitVector::<1>::new(0b1))))}
        (b__5, Privilege::User) if {(b__5 == BitVector::<12>::new(0b110000000010))} => {((_get_Counteren_IR(sail_ctx, sail_ctx.mcounteren) == BitVector::<1>::new(0b1)) && (!(haveSupMode(sail_ctx, ())) || (_get_Counteren_IR(sail_ctx, sail_ctx.scounteren) == BitVector::<1>::new(0b1))))}
        (_, _) => {if {(_operator_smallerequal_u_(sail_ctx, BitVector::<12>::new(0b110000000011), csr) && _operator_smallerequal_u_(sail_ctx, csr, BitVector::<12>::new(0b110000011111)))} {
            false
        } else {
            true
        }}
...
pub fn check_seed_CSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege, isWrite: bool) -> bool {
...
pub fn check_CSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, p: Privilege, isWrite: bool) -> bool {
    (is_CSR_defined(sail_ctx, csr, p) && ({
        let var_244 = csrAccess(sail_ctx, csr);
        let var_245 = csrPriv(sail_ctx, csr);
        let var_246 = p;
        let var_247 = isWrite;
        check_CSR_access(sail_ctx, var_244, var_245, var_246, var_247)
    } && (check_TVM_SATP(sail_ctx, csr, p) && (check_Counteren(sail_ctx, csr, p) && check_seed_CSR(sail_ctx, csr, p, isWrite)))))
}
...
pub fn exception_delegatee(sail_ctx: &mut SailVirtCtx, e: ExceptionType, p: Privilege) -> Privilege {
    ...
    let _super_ = {
        let var_251 = bitvector_access(sail_ctx.medeleg.bits, idx);
        bit_to_bool(sail_ctx, var_251)
    };
    let user = if {haveSupMode(sail_ctx, ())} {
        (_super_ && (haveNExt(sail_ctx, ()) && {
            let var_250 = bitvector_access(sail_ctx.sedeleg.bits, idx);
            bit_to_bool(sail_ctx, var_250)
        }))
    } else {
        (_super_ && haveNExt(sail_ctx, ()))
    };
    ...
    if {{
        let var_248 = privLevel_to_bits(sail_ctx, deleg);
        let var_249 = privLevel_to_bits(sail_ctx, p);
        _operator_smaller_u_(sail_ctx, var_248, var_249)
    }} {
        p
    } else {
        deleg
...
pub fn trap_handler(sail_ctx: &mut SailVirtCtx, del_priv: Privilege, intr: bool, c: BitVector<8>, pc: BitVector<64>, info: Option<BitVector<64>>, ext: Option<()>) -> BitVector<64> {
    rvfi_trap(sail_ctx, ());
    ...
    match del_priv {
        Privilege::Machine => {{
            sail_ctx.mcause = {
                let var_252 = bool_to_bits(sail_ctx, intr);
                sail_ctx.mcause.set_subrange::<63, 64, 1>(var_252)
            };
            sail_ctx.mcause = {
                let var_253 = zero_extend_63(c);
                sail_ctx.mcause.set_subrange::<0, 63, 63>(var_253)
            };
            sail_ctx.mstatus = {
                let var_254 = _get_Mstatus_MIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<7, 8, 1>(var_254)
            };
            sail_ctx.mstatus = sail_ctx.mstatus.set_subrange::<3, 4, 1>(BitVector::<1>::new(0b0));
            sail_ctx.mstatus = {
                let var_255 = privLevel_to_bits(sail_ctx, sail_ctx.cur_privilege);
                sail_ctx.mstatus.set_subrange::<11, 13, 2>(var_255)
            };
        ...
        Privilege::Supervisor => {{
            ...
            sail_ctx.scause = {
                let var_256 = bool_to_bits(sail_ctx, intr);
                sail_ctx.scause.set_subrange::<63, 64, 1>(var_256)
            };
            sail_ctx.scause = {
                let var_257 = zero_extend_63(c);
                sail_ctx.scause.set_subrange::<0, 63, 63>(var_257)
            };
            sail_ctx.mstatus = {
                let var_258 = _get_Mstatus_SIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<5, 6, 1>(var_258)
            };
            ...
            sail_ctx.mstatus = sail_ctx.mstatus.set_subrange::<8, 9, 1>(match sail_ctx.cur_privilege {
                ...
                Privilege::Machine => {internal_error(String::from("../miralis-sail-riscv/model/riscv_sys_control.sail"), 359, String::from("invalid privilege for s-mode trap"))}
            });
        ...
        Privilege::User => {{
            ...
            sail_ctx.ucause = {
                let var_259 = bool_to_bits(sail_ctx, intr);
                sail_ctx.ucause.set_subrange::<63, 64, 1>(var_259)
            };
            sail_ctx.ucause = {
                let var_260 = zero_extend_63(c);
                sail_ctx.ucause.set_subrange::<0, 63, 63>(var_260)
            };
            sail_ctx.mstatus = {
                let var_261 = _get_Mstatus_UIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<4, 5, 1>(var_261)
            };
...
pub fn exception_handler(sail_ctx: &mut SailVirtCtx, cur_priv: Privilege, ctl: ctl_result, pc: BitVector<64>) -> BitVector<64> {
    match (cur_priv, ctl) {
        (_, ctl_result::CTL_TRAP(e)) => {{
            let del_priv = {
                let var_270 = e.trap;
                let var_271 = cur_priv;
                exception_delegatee(sail_ctx, var_270, var_271)
            };
            if {get_config_print_platform(sail_ctx, ())} {
                print_platform(format!("{}{}", String::from("trapping from "), format!("{}{}", privLevel_to_str(sail_ctx, cur_priv), format!("{}{}", String::from(" to "), format!("{}{}", privLevel_to_str(sail_ctx, del_priv), format!("{}{}", String::from(" to handle "), {
                    let var_262 = e.trap;
                    exceptionType_to_str(sail_ctx, var_262)
                }))))))
            } else {
                ()
            };
            {
                let var_263 = del_priv;
                let var_264 = false;
                let var_265 = {
                    let var_269 = e.trap;
                    exceptionType_to_bits(sail_ctx, var_269)
                };
                let var_266 = pc;
                let var_267 = e.excinfo;
                let var_268 = e.ext;
                trap_handler(sail_ctx, var_263, var_264, var_265, var_266, var_267, var_268)
            }
        }}
        (_, ctl_result::CTL_MRET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_272 = _get_Mstatus_MPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<3, 4, 1>(var_272)
            };
            sail_ctx.mstatus = sail_ctx.mstatus.set_subrange::<7, 8, 1>(BitVector::<1>::new(0b1));
            sail_ctx.cur_privilege = {
                let var_273 = _get_Mstatus_MPP(sail_ctx, sail_ctx.mstatus);
                privLevel_of_bits(sail_ctx, var_273)
            };
            sail_ctx.mstatus = {
                let var_274 = {
                    let var_275 = if {haveUsrMode(sail_ctx, ())} {
                        Privilege::User
                    } else {
                        Privilege::Machine
                    };
                    privLevel_to_bits(sail_ctx, var_275)
                };
                sail_ctx.mstatus.set_subrange::<11, 13, 2>(var_274)
            };
        ...
        (_, ctl_result::CTL_SRET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_276 = _get_Mstatus_SPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<1, 2, 1>(var_276)
            };
        ...
        (_, ctl_result::CTL_URET(())) => {{
            let prev_priv = sail_ctx.cur_privilege;
            sail_ctx.mstatus = {
                let var_277 = _get_Mstatus_UPIE(sail_ctx, sail_ctx.mstatus);
                sail_ctx.mstatus.set_subrange::<0, 1, 1>(var_277)
            };
...
pub fn handle_illegal(sail_ctx: &mut SailVirtCtx, unit_arg: ()) {
    ...
    {
        let var_278 = {
            let var_279 = sail_ctx.cur_privilege;
            let var_280 = ctl_result::CTL_TRAP(t);
            let var_281 = sail_ctx.PC;
            exception_handler(sail_ctx, var_279, var_280, var_281)
        };
        set_next_pc(sail_ctx, var_278)
    }
}
...
pub fn legalize_satp(sail_ctx: &mut SailVirtCtx, a: Architecture, o: BitVector<64>, v: BitVector<64>) -> BitVector<64> {
    if {(64 == 32)} {
        panic!("unreachable code")
    } else if {(64 == 64)} {
        ...
        internal_error(String::from("../miralis-sail-riscv/model/riscv_vmem.sail"), 205, format!("{}{}", String::from("Unsupported xlen"), dec_str(64)))
    }
}
...
pub fn readCSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>) -> BitVector<64> {
    let res: xlenbits = match (csr, 64) {
        ...
        (v__12, _) if {{
            ...
            let var_282 = idx.as_usize();
            pmpReadCfgReg(sail_ctx, var_282)}
        (v__14, _) if {(v__14.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111011))} => {let idx: BitVector<4> = v__14.subrange::<0, 4, 4>();
            let var_283 = bitvector_concat(BitVector::<2>::new(0b00), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_283)}
        (v__16, _) if {(v__16.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111100))} => {let idx: BitVector<4> = v__16.subrange::<0, 4, 4>();
            let var_284 = bitvector_concat(BitVector::<2>::new(0b01), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_284)}
        (v__18, _) if {(v__18.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111101))} => {let idx: BitVector<4> = v__18.subrange::<0, 4, 4>();
            let var_285 = bitvector_concat(BitVector::<2>::new(0b10), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_285)}
        (v__20, _) if {(v__20.subrange::<4, 12, 8>() == BitVector::<8>::new(0b00111110))} => {let idx: BitVector<4> = v__20.subrange::<0, 4, 4>();
            let var_286 = bitvector_concat(BitVector::<2>::new(0b11), idx).as_usize();
            pmpReadAddrReg(sail_ctx, var_286)}
...
pub fn writeCSR(sail_ctx: &mut SailVirtCtx, csr: BitVector<12>, value: BitVector<64>) {
    let res: Option<xlenbits> = match (csr, 64) {
        ...
        (b__28, _) if {(b__28 == BitVector::<12>::new(0b000100001010))} => {{
            sail_ctx.senvcfg = {
                let var_291 = sail_ctx.senvcfg;
                let var_292 = zero_extend_64(value);
                legalize_senvcfg(sail_ctx, var_291, var_292)
            };
        ...
        (b__34, _) if {(b__34 == BitVector::<12>::new(0b000110000000))} => {{
            sail_ctx.satp = {
                let var_293 = cur_Architecture(sail_ctx, ());
                let var_294 = sail_ctx.satp;
                let var_295 = value;
                legalize_satp(sail_ctx, var_293, var_294, var_295)
            };
            Some(sail_ctx.satp)
        }}
...
pub fn execute_ITYPE(sail_ctx: &mut SailVirtCtx, imm: BitVector<12>, rs1: regidx, rd: regidx, op: iop) -> Retired {
    ...
    let result: xlenbits = match op {
        ...
        iop::RISCV_SLTI => {zero_extend_64({
            let var_296 = _operator_smaller_s_(sail_ctx, rs1_val, immext);
            bool_to_bits(sail_ctx, var_296)
        })}
        iop::RISCV_SLTIU => {zero_extend_64({
            let var_297 = _operator_smaller_u_(sail_ctx, rs1_val, immext);
            bool_to_bits(sail_ctx, var_297)
        })}
...
pub fn execute_MRET(sail_ctx: &mut SailVirtCtx) -> Retired {
    if {(sail_ctx.cur_privilege != Privilege::Machine)} {
        ...
        {
            let var_298 = {
                let var_299 = sail_ctx.cur_privilege;
                let var_300 = ctl_result::CTL_MRET(());
                let var_301 = sail_ctx.PC;
                exception_handler(sail_ctx, var_299, var_300, var_301)
            };
            set_next_pc(sail_ctx, var_298)
        };
        Retired::RETIRE_SUCCESS
    }
}
...
pub fn execute_CSR(sail_ctx: &mut SailVirtCtx, csr: csreg, rs1: regidx, rd: regidx, is_imm: bool, op: csrop) -> Retired {
