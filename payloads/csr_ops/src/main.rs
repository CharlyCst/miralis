...
fn test_csr_op(regs: &[(usize, usize, usize)]) {
    ...
    for (in_rd, in_rs1, in_csr) in regs {
        ...
        check_csrrw(*in_rd, *in_rs1, *in_csr, out_csr, out_rd);
    }
    // CSRRS
    for (in_rd, in_rs1, in_csr) in regs {
        let (out_csr, out_rd) = unsafe { csrrs(*in_csr, *in_rd, *in_rs1) };
        check_csrrs(*in_rd, *in_rs1, *in_csr, out_csr, out_rd);
    }
}

fn check_csrrw(_in_rd: usize, in_rs1: usize, in_csr: usize, out_csr: usize, out_rd: usize) {
...
fn check_csrrs(_in_rd: usize, in_rs1: usize, in_csr: usize, out_csr: usize, out_rd: usize) {
